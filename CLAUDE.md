...
**Header dependencies are generated by the compiler** (`-MMD -MP`, one
`.d` beside each object and each linked binary, `-include`d from
`rules.mk`). There were none until recently: nothing knew that
`ai-claude-client.o` includes `ai-response.h`, so editing *any* header
left every object that included it stale and `make` said there was
nothing to do. That is the silent kind — it survives a test run, so a
change to a header could be "verified" against a binary that never
contained it. If you are working from a tree whose objects predate this,
one `rm -rf build/<type>/obj build/<type>/tests build/<type>/bin` seeds
the `.d` files; after that it is incremental as usual. Tests live under `build/<type>/tests/`, examples under
...
4. **`!`cmd`` in a command body is opt-in per file** (`shell: true`).
   ...
   **The opt-in is about the file, so only the file's text is scanned.**
   `ai_command_set_resolve()` interpolated `$ARGUMENTS` and `$1`..`$9`
   into the body and handed the *result* to the scanner, so a caller
   could pass `` !`id` `` as an argument and have it run by a file
   containing no backticks at all — `/summarise !`id -un`` against a body
   of "Please summarise: $ARGUMENTS" printed the username. `expand_body()`
   walks the body once: the scan sees only the file, argument text is
   appended as literal text, and command output is appended without a
   second placeholder pass (or `` !`echo 'costs $5'` `` loses the `$5` to
   an empty positional).

   A body's own command still gets the arguments, because a fix that took
   that away would have been a different feature: `run_shell()` passes
   them as the shell's positional parameters and `$ARGUMENTS` in its
   environment. A parameter the shell expands is one word and is never
   re-parsed, so `` !`echo $1` `` invoked with `x; echo PWNED` echoes one
   word instead of running two commands — which the previous spelling,
   pasting the argument into the script text, did.
...
as"**. `"usage": null` and `"content": {}` both pass it, and json-glib's
typed accessor then logs a `Json-CRITICAL` and returns NULL — which is
handed straight to the next accessor for a second one. Under GTest and
...
ai_json_get_array(obj, "content")
ai_json_get_string(obj, "id", "")           /* fallback may be NULL   */
ai_json_get_int(obj, "input_tokens", 0)     /* int, int64 or double   */
ai_json_get_double(obj, "cost", 0.0)
ai_json_get_boolean(obj, "done", FALSE)
ai_json_get_node(obj, "input")              /* pass a value through   */
ai_json_array_get_object(arr, i)            /* NULL-safe in both args */
ai_json_root_object(parser)                 /* the bare `null` guard  */
```
