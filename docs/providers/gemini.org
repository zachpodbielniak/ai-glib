...
=gemini-3-pro-preview= was retired upstream on 2026-03-09; migrate to =AI_GEMINI_MODEL_3_1_PRO_PREVIEW=.
...
Identifiers only. =AiGeminiClient= does not yet expose Live, native-audio, or TTS endpoints; these constants exist so callers can reference upstream IDs by symbolic name once support lands.
...
- *Tool Use*: Full support (function declarations, function calls, function responses, including streaming)
...
- Assistant tool calls arrive as =parts:[{functionCall:{name, args}}]= inside a =role:"model"= content item. =args= is a parsed JSON object, not a string.
- Tool results must be sent back as =parts:[{functionResponse:{name, response:{output|error}}}]= in a =role:"user"= content item. Crucially, Gemini keys the response by =name=, not by a call id.

Two things to know when calling tools through Gemini:

1. Use =ai_message_new_tool_result_with_name(id, tool_name, content, is_error)= when sending results back. The tool name is required on the wire. If you pass =NULL= for the name, ai-glib falls back to scanning earlier assistant messages for a matching =tool_use_id= --- works for simple loops but is slower and less robust than just supplying it.
2. Gemini's API does not return tool-call IDs. ai-glib synthesizes a UUID per call so the multi-turn loop can match results back to calls internally. The synthetic ID is only used in-memory; on the wire only the tool name matters.
...
** Image Generation
*** Capability matrix
| Model                          | Refs | Max n | Geometry              | Notable                          |
|--------------------------------+------+-------+-----------------------+----------------------------------|
| =gemini-2.5-flash-image=       |    3 |     4 | aspect ratio          | seed, sampling, safety           |
| =gemini-3.1-flash-image-preview= |  6 |     4 | aspect ratio          | multi-reference                  |
| =gemini-3-pro-image-preview=   |   14 |     4 | aspect ratio + tier   | multi-reference, up to 4K        |
| =imagen-4.0-generate-001=      |    0 |     4 | aspect ratio          | negative prompt, watermark, language |
| =imagen-3.0-generate-001=      |    0 |     4 | aspect ratio          | negative prompt, watermark       |

Gemini expresses geometry as an aspect ratio, not pixel dimensions, so
=ai_image_request_set_aspect_ratio()= is the natural control here;
=ai_image_request_set_size()= is dropped for these models. Nano Banana Pro adds
a resolution tier (=1K=/=2K=/=4K=).

Only the Nano Banana models accept reference images. They run as chat
completions under =:generateContent=, which is what gives them somewhere to put
extra image parts -- Imagen's =:predict= endpoint is purely generative and
rejects references outright.

Supported ratios: =1:1=, =2:3=, =3:2=, =3:4=, =4:3=, =4:5=, =5:4=, =9:16=,
=16:9=, =21:9= for Nano Banana; Imagen accepts the first five plus =9:16= and
=16:9=.

*** Wire-format notes
- Requests spell inline image data snake_case (=inline_data=); responses come
  back camelCase (=inlineData=). Both are handled.
- The API key travels in the =x-goog-api-key= header rather than the query
  string, so it does not leak into proxy logs.
- Three different response envelopes are in circulation (=predictions=,
  =generatedImages=, =candidates=); all three are parsed.
- If the model answers with text instead of an image -- which it will if asked
  for something it declines to draw -- that surfaces as
  =AI_ERROR_INVALID_RESPONSE= rather than an empty success.
...
- [[https://ai.google.dev/docs][Gemini API Documentation]]
- [[https://ai.google.dev/models/gemini][Gemini Models]]
