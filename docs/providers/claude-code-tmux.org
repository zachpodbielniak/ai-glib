...
| =AI_CLAUDE_TMUX_MODEL_OPUS_5_5= | ="claude-opus-5-5"= |
...
the pinned Claude 5 and Opus 5.5 IDs. Opus 5.5's id is =claude-opus-5-5=.
...
=sandbox= accepts =enabled= or =disabled=; NULL preserves existing behavior.
For example, =ai -p claude-tmux --sandbox enabled "Run checks"= merges the
native Bash sandbox configuration into the generated settings file while
preserving its SessionStart and Stop hooks. Enabling sets
=allowUnsandboxedCommands=false= and =failIfUnavailable=true= as well as
=enabled=true=. An invalid mode is rejected before starting tmux. Permission
approval remains a separate setting. The dry-run command shows the generated
settings file placeholder, not its eventual JSON contents.
See [[file:../cli.org][CLI sandbox policies]] for supported provider values and scope.
