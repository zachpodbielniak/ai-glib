...
** Provider Comparison
| Feature          | Claude | OpenAI | Gemini | Grok | Ollama                | Claude Code        | Claude Code (tmux) | OpenCode           | Grok Build         |
|------------------+--------+--------+--------+------+-----------------------+--------------------+--------------------+--------------------+--------------------|
| Chat Completion  | Yes    | Yes    | Yes    | Yes  | Yes                   | Yes                | Yes                | Yes                | Yes                |
| Streaming        | Yes    | Yes    | Yes    | Yes  | Yes                   | Yes                | No                 | Yes                | Yes                |
| Tool Use         | Yes    | Yes    | Yes    | Yes  | Yes (model-dependent) | Yes                | Yes                | Model-dependent    | Yes (CLI built-in) |
| Vision           | Yes    | Yes    | Yes    | Yes  | Model-dependent       | Yes                | Yes                | Model-dependent    | No                 |
| Local            | No     | No     | No     | No   | Yes                   | No                 | No                 | No                 | No                 |
| API Key Required | Yes    | Yes    | Yes    | Yes  | No                    | No (uses CLI auth) | No (uses CLI auth) | No (uses CLI auth) | No (uses CLI auth) |
| Multi-Provider   | No     | No     | No     | No   | No                    | No                 | No                 | Yes                | No                 |
| Billing Path     | API    | API    | API    | API  | n/a                   | Agent SDK          | Subscription (TUI) | Agent SDK          | xAI account        |
