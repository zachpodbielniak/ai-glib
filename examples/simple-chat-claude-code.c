/*
 * simple-chat-claude-code.c - Simple chat example using Claude Code CLI
 *
 * Copyright (C) 2025
 * SPDX-License-Identifier: AGPL-3.0-or-later
 *
 * This example demonstrates basic usage of ai-glib with the Claude Code CLI.
 * It sends a simple message and prints the response.
 *
 * Requirements:
 *   - The `claude` CLI must be installed and configured
 *   - Either in PATH or set CLAUDE_CODE_PATH environment variable
 *
 * Usage:
 *   ./simple-chat-claude-code [prompt]
 */
...
on_chat_complete(
	...
	cost = ai_claude_code_client_get_total_cost(AI_CLAUDE_CODE_CLIENT(source));
	if (cost > 0.0)
	{
		printf("Cost: $%.6f USD\n", cost);
	}
...
main(
	...
	/* Create client (uses `claude` CLI from PATH or CLAUDE_CODE_PATH) */
