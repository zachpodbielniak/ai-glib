...
/*
 * The kernel accepts the argv this client builds -- and refuses the one
 * it used to build.
 *
 * Every other test here asserts on the *shape* of build_argv()'s
 * output: no word over the limit, the flags present, the files right.
 * None of them proves the limit is what this codebase believes it is,
 * because none of them calls execve.  The bug was not a wrong string,
 * it was a spawn that failed, and a test of the string is a test of our
 * own arithmetic about a number the kernel owns.
 *
 * So this spawns twice, with /bin/true, which does nothing and exits 0:
 * once with a 200KB argument, which must fail with E2BIG, and once with
 * a path to a 200KB file, which must succeed.  The first half is what
 * makes the second mean anything -- without it, a machine whose limit
 * was different, or removed, would pass this test while proving that
 * the fix was never needed.
 */
static void
test_the_kernel_refuses_the_old_shape(void)
{
	g_autofree gchar *huge = huge_prompt('K');
	g_autofree gchar *path = NULL;
	g_autoptr(GError) inline_error = NULL;
	g_autoptr(GError) file_error = NULL;
	g_autoptr(GSubprocess) inline_spawn = NULL;
	g_autoptr(GSubprocess) file_spawn = NULL;
	gint fd;

	g_assert_cmpuint(strlen(huge), >=, AI_TEST_MAX_ARG_STRLEN);

	/* The old shape: the whole prompt as one argv word. */
	inline_spawn = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_SILENCE |
	                                G_SUBPROCESS_FLAGS_STDERR_SILENCE,
	                                &inline_error,
	                                "/bin/true", "--system-prompt", huge,
	                                NULL);

	if (inline_spawn != NULL)
		g_error("the kernel accepted a %" G_GSIZE_FORMAT "-byte argv "
		        "word; this machine's MAX_ARG_STRLEN is not the %d "
		        "this test is written around, so nothing below it is "
		        "evidence", strlen(huge), AI_TEST_MAX_ARG_STRLEN);

	g_assert_nonnull(inline_error);

	/*
	 * The new shape: a path.  Written through the descriptor rather
	 * than by name, the same way the client does it.
	 */
	fd = g_file_open_tmp("ai-glib-argv-limit-XXXXXX", &path, &file_error);
	g_assert_no_error(file_error);
	g_assert_cmpint(fd, >=, 0);
	g_assert_cmpint((gint)write(fd, huge, strlen(huge)), ==,
	                (gint)strlen(huge));
	g_assert_cmpint(close(fd), ==, 0);

	file_spawn = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_SILENCE |
	                              G_SUBPROCESS_FLAGS_STDERR_SILENCE,
	                              &file_error,
	                              "/bin/true", "--system-prompt-file", path,
	                              NULL);

	g_assert_no_error(file_error);
	g_assert_nonnull(file_spawn);
	g_assert_true(g_subprocess_wait(file_spawn, NULL, NULL));
