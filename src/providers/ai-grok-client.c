...
ai_grok_client_parse_response(
    ...
    /*
     * Presence, not shape: a server that answers `"error": "boom"` has
     * still said it failed, and reading the message off a NULL object
     * gives the generic wording rather than a pair of criticals.
     */
...
grok_process_stream_chunk(
...

    {
        ...
        if (choice != NULL)
        {
            /* A chunk mid-stream sends JSON null here, which the
             * accessor reports as absent. */
            const gchar *finish_reason =
