/*
 * ai-grok-build-client-internal.h - test/introspection seam for the
 *                                   Grok Build CLI client
 *
 * Copyright (C) 2025
...
#error "ai-grok-build-client-internal.h is an internal header"
#endif
...
/*
 * Build the argv for the grok CLI. The returned array is NULL-terminated;
 * free with g_strfreev().
 *
 * @messages: (element-type AiMessage): conversation messages (used only
 *   for shape; the prompt itself is piped via stdin and read back through
 *   `--prompt-file /dev/stdin`, not passed in argv).
 * @system_prompt: (nullable): system prompt for a fresh session.
 * @streaming: TRUE to request streaming-messages-json output.
 *
