/*
 * ai-antigravity-client-internal.h - test/introspection seam for the
...
#error "ai-antigravity-client-internal.h is an internal header"
#endif
...
/*
 * Build the argv for the agy CLI. The returned array is NULL-terminated;
 * free with g_strfreev().
 *
 * @messages: (element-type AiMessage): conversation messages (used only
 *   for shape; the prompt itself is piped via stdin as a stream-json
 *   user event, not passed in argv).
 * @system_prompt: (nullable): system prompt for a fresh session.
 * @streaming: ignored -- print mode always uses --output-format
 *   stream-json because --input-format stream-json requires it.
 *
